...
def transcribe(audio_path, model, decoder, device, audio_conf, normalize=True):
    parser = DataProcessor(audio_conf, normalize=normalize)
    # spect = parse_audio(audio_path,
    #                     int(audio_conf["sample_rate"] * audio_conf["window_stride"]),
    #                     int(audio_conf["sample_rate"] * audio_conf["window_stride"]),
    #                     torch.hamming_window, normalize
    #                     ).contiguous()
    spect = parser.parse_audio(audio_path)
...
if __name__ == '__main__':
    ...
    decoded_output, decoded_offsets = transcribe(args.audio_path, model, decoder, device, model.audio_conf, normalize=True)
