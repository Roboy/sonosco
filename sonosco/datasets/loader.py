...
class AudioDataLoaderWavOnly(DataLoader):

    def __init__(self, *args, **kwargs):
        '''
        Creates a data loader for AudioDatasets.
        '''
        super(AudioDataLoaderWavOnly, self).__init__(*args, **kwargs)
        self.collate_fn = self._collate_fn
    ...
    def _collate_fn(self, batch):
        ...
        # get filenames of the audio files
        filenames = list(zip(*batch))[1]  # extract filenames array from batch (batch is array of tuples)

        return inputs, input_lengths, filenames
